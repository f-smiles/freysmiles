...
uniform float u_turbulanceIntensity;
...
void main() {
  ...
  float ration = u_imageAspect / screenAspect;

  vec2 texCoord = vec2(
    mix(0.5 - 0.5 / ration, 0.5 + 0.5 / ration, uv.x),
    uv.y
  );
  ...
  float jaggedDist = dist + (turbulence(uv * 25.0 + u_time * u_speed) - 0.5) * u_turbulanceIntensity;
